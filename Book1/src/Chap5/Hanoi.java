...
public class Hanoi {
    static void originMove(int no, int x, int y) {
        if (no > 1)
            originMove(no - 1, x, 6 - x - y);
        System.out.printf("원반 [%d]를 %d번 기둥에서 %d번 기둥으로 옮김\n", no, x, y);
        if (no > 1)
            originMove(no - 1, 6 - x - y, y);
    }

    static void noRecurMove(int no, int x, int y) {
    ...
    static void abcMove(int no, int x, int y) {
        String[] name = {"A", "B", "C"};
        if (no > 1)
            abcMove(no - 1, x, 6 - x - y);
        System.out.println("원반[" + no + "]을 " + name[x - 1] + "에서 " + name[y - 1] + "으로 이동");
        if (no > 1)
            abcMove(no - 1, 6 - x - y, y);
    }
